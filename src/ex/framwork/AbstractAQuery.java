...
import ex.framwork.auth.AccountHandle;
import ex.framwork.callback.AbstractAjaxCallback;
import ex.framwork.callback.AjaxCallback;
import ex.framwork.callback.BitmapAjaxCallback;
import ex.framwork.callback.ImageOptions;
import ex.framwork.callback.Transformer;
import ex.framwork.util.ExUtility;
import ex.framwork.util.Common;
import ex.framwork.util.Constants;
import ex.framwork.util.WebImage;
...
public abstract class AbstractAQuery<T extends AbstractAQuery<T>> implements Constants {
	...
	protected T image(String url, boolean memCache, boolean fileCache, int targetWidth, int fallbackId, Bitmap preset, int animId, float ratio, int round, String networkUrl){
		...
		if(view instanceof ImageView){		
			BitmapAjaxCallback.async(act, getContext(), (ImageView) view, url, memCache, fileCache, targetWidth, fallbackId, preset, animId, ratio, Ex.ANCHOR_DYNAMIC, progress, ah, policy, round, proxy, networkUrl);			
			reset();
		}
	...
	public T transparent(boolean transparent){
		...
		if(view != null){
			ExUtility.transparent(view, transparent);
		}
	...
	private Common setScrollListener(){
		...
		Common common = (Common) lv.getTag(Ex.TAG_SCROLL_LISTENER);
		if(common == null){
			common = new Common();
			lv.setOnScrollListener(common);
			lv.setTag(Ex.TAG_SCROLL_LISTENER, common);
			ExUtility.debug("set scroll listenr");
		}
	...
	public T overridePendingTransition5(int enterAnim, int exitAnim){
		...
		if(act != null){
			ExUtility.invokeHandler(act, "overridePendingTransition", false, false, PENDING_TRANSITION_SIG, enterAnim, exitAnim);
		}
	...
	public T setOverScrollMode9(int mode){
		...
		if(view instanceof AbsListView){
			ExUtility.invokeHandler(view, "setOverScrollMode", false, false, OVER_SCROLL_SIG, mode);
		}
	...
	public T setLayerType11(int type, Paint paint){
		...
		if(view != null){
			...
			ExUtility.invokeHandler(view, "setLayerType", false, false, LAYER_TYPE_SIG, type, paint);
		}
	...
	public Object invoke(String method, Class<?>[] sig, Object... params){
		...
		return ExUtility.invokeHandler(handler, method, false, false, sig, params);
	}
	...
	public T hardwareAccelerated11(){
		...
		if(act != null){
			act.getWindow().setFlags(Ex.FLAG_HARDWARE_ACCELERATED, Ex.FLAG_HARDWARE_ACCELERATED);
		}
	...
	public T margin(float leftDip, float topDip, float rightDip, float bottomDip){
		...
		if(view != null){
			...
			if(lp instanceof MarginLayoutParams){
				...
				int left = ExUtility.dip2pixel(context, leftDip);
				int top = ExUtility.dip2pixel(context, topDip);
				int right = ExUtility.dip2pixel(context, rightDip);
				int bottom = ExUtility.dip2pixel(context, bottomDip);
	...
	private void size(boolean width, int n, boolean dip){
		...
		if(view != null){
			...
			if(n > 0 && dip){
				n = ExUtility.dip2pixel(context, n);
			}
	...
	public <K> T delete(String url, Class<K> type, AjaxCallback<K> callback){
		...
		callback.url(url).type(type).method(Ex.METHOD_DELETE);		
		return ajax(callback);
	...
	public <K> T put(String url, String contentHeader, HttpEntity entity, Class<K> type, AjaxCallback<K> callback){
		...
		callback.url(url).type(type).method(Ex.METHOD_PUT).header("Content-Type", contentHeader).param(Ex.POST_ENTITY, entity);		
		return ajax(callback);
	...
	public <K> T post(String url, String contentHeader, HttpEntity entity, Class<K> type, AjaxCallback<K> callback){
        ...
        callback.url(url).type(type).method(Ex.METHOD_POST).header("Content-Type", contentHeader).param(Ex.POST_ENTITY, entity);     
        return ajax(callback);
	...
	public File getCachedFile(String url){
		...
		File result = ExUtility.getExistedCacheByUrl(ExUtility.getCacheDir(getContext(), Ex.CACHE_PERSISTENT), url);
		if(result == null) result = ExUtility.getExistedCacheByUrl(ExUtility.getCacheDir(getContext(), Ex.CACHE_DEFAULT), url);
		return result;
	}
	...
	public File makeSharedFile(String url, String filename){
		...
		try{
			...
			if(cached != null){
				...
				File temp = ExUtility.getTempDir();
	...
	public T webImage(String url, boolean zoom, boolean control, int color){
		...
		if(view instanceof WebView){
			setLayerType11(Ex.LAYER_TYPE_SOFTWARE, null);
	...
	public View inflate(View convertView, int layoutId, ViewGroup root){
		...
		if(convertView != null){
			Integer layout = (Integer) convertView.getTag(Ex.TAG_LAYOUT);
		...
		view.setTag(Ex.TAG_LAYOUT, layoutId);
