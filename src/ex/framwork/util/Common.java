...
public class Common implements Comparator<File>, Runnable, OnClickListener, OnLongClickListener, OnItemClickListener, OnScrollListener, OnItemSelectedListener, TextWatcher{
	...
	private Object invoke(Object... args){
		...
		if(method != null){
			...
			Object result = ExUtility.invokeHandler(cbo, method, fallback, true, sig, input);
			return result;
		}else if(methodId != 0){
			...
			switch(methodId){
				...
				case CLEAN_CACHE:
					ExUtility.cleanCache((File) params[0], (Long) params[1], (Long) params[2]);
					break;
				case STORE_FILE:
					ExUtility.store((File) params[0], (byte[]) params[1]);
					break;
	...
	private void onScrollStateChanged(ExpandableListView elv, int scrollState){
		...
		elv.setTag(Ex.TAG_NUM, scrollState);
		...
		if(scrollState == SCROLL_STATE_IDLE){
			...
			for(int i = 0; i <= count; i++){
				...
				if(group >= 0){
					...
					Long targetPacked = (Long) convertView.getTag(Ex.TAG_NUM);
					...
					if(targetPacked != null && targetPacked.longValue() == packed){
						...
						convertView.setTag(Ex.TAG_NUM, null);
					}else{
	...
	private void onScrollStateChanged2(AbsListView lv, int scrollState){
		...
		lv.setTag(Ex.TAG_NUM, scrollState);
		...
		if(scrollState == SCROLL_STATE_IDLE){
			...
			for(int i = 0; i <= count; i++){
				...
				Number targetPacked = (Number) convertView.getTag(Ex.TAG_NUM);
	...
	public static boolean shouldDelay(int groupPosition, int childPosition, View convertView, ViewGroup parent, String url){
		...
		OnScrollListener sl = (OnScrollListener) parent.getTag(Ex.TAG_SCROLL_LISTENER);
		
		if(sl == null){
			sl = new Common();
			lv.setOnScrollListener(sl);
			parent.setTag(Ex.TAG_SCROLL_LISTENER, sl);
		}
		
		Integer scrollState = (Integer) lv.getTag(Ex.TAG_NUM);
		...
		convertView.setTag(Ex.TAG_NUM, packed);
	...
	private static boolean shouldDelayGallery(int position, View convertView, ViewGroup parent, String url){
		...
		Integer selected = (Integer) gallery.getTag(Ex.TAG_NUM);
		
		if(selected == null){
			
			selected = 0;
			gallery.setTag(Ex.TAG_NUM, 0);
		...
		if((position >= from && position <= to)){
			...
			convertView.setTag(Ex.TAG_NUM, position);
	...
	public void onItemSelected(AdapterView<?> parent, View v, int pos, long id) {
		...
		if(galleryListen){
			...
			Integer selected = (Integer) parent.getTag(Ex.TAG_NUM);
			
			if(selected != pos){
			
				Adapter adapter = parent.getAdapter();
				parent.setTag(Ex.TAG_NUM, pos);
				...
				for(int i = 0; i < count; i++){
					...
					Integer lastDrawn = (Integer) convertView.getTag(Ex.TAG_NUM);
