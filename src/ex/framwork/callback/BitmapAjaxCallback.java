...
import ex.framwork.util.BitmapCache;
import ex.framwork.util.Common;
import ex.framwork.util.RatioDrawable;
...
public class BitmapAjaxCallback extends AbstractAjaxCallback<Bitmap, BitmapAjaxCallback>{
	...
	private float anchor = Ex.ANCHOR_DYNAMIC;
	...
	private static Bitmap decode(String path, byte[] data, BitmapFactory.Options options, boolean rotate){
		...
		if(result == null && options != null && !options.inJustDecodeBounds){
			ExUtility.debug("decode image failed", path);
		}
	...
	private static Bitmap rotate(String path, Bitmap bm){
		...
		if(ori > 0){
			...
			ExUtility.debug("before", bm.getWidth() + ":" + bm.getHeight());
			ExUtility.debug("after", result.getWidth() + ":" + result.getHeight());
	...
	public Bitmap transform(String url, byte[] data, AjaxStatus status) {
		...
		if(bm == null){
			...
			if(fallback > 0){			
				bm = getFallback();		
			}else if(fallback == Ex.GONE || fallback == Ex.INVISIBLE){
				bm = dummy;
			}else if(fallback == Ex.PRESET){
				bm = preset;
			}
			...
			if(status.getSource() == AjaxStatus.NETWORK && file != null){
			    ExUtility.debug("invalid bm from net");
			    file.delete();
			}
	...
	private void checkCb(BitmapAjaxCallback cb, String url, ImageView v, Bitmap bm, AjaxStatus status){
		...
		if(url.equals(v.getTag(Ex.TAG_URL))){			
	...
	private static Bitmap filter(View iv, Bitmap bm, int fallback){
		...
		}else if(fallback == Ex.GONE){
			iv.setVisibility(View.GONE);
		}else if(fallback == Ex.INVISIBLE){
	...
	private void presetBitmap(String url, ImageView v){
		...
		if(!url.equals(v.getTag(Ex.TAG_URL)) || preset != null){			
			
			v.setTag(Ex.TAG_URL, url);
	...
	private static boolean fadeIn(int animation, int source){
		...
		switch(animation){
			case Ex.FADE_IN:
				return true;
			case Ex.FADE_IN_FILE:
				if(source == AjaxStatus.FILE) return true;		
			case Ex.FADE_IN_NETWORK:
