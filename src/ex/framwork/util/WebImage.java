...
public class WebImage extends WebViewClient{
	...
	private static String getSource(Context context){
		...
		if(template == null){
			...
			try{
				...
				template = new String(ExUtility.toBytes(is));
			}catch(Exception e){
				ExUtility.debug(e);
			}
	...
	public void load(){
		...
		if(url.equals(wv.getTag(Ex.TAG_URL))){
    		return;
    	}
		
    	wv.setTag(Ex.TAG_URL, url);
	...
	private static void disableZoomControl(WebView wv){
		...
		WebSettings ws = wv.getSettings();		
		ExUtility.invokeHandler(ws, "setDisplayZoomControls", false, false, new Class[]{boolean.class}, false);
